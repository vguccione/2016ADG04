...
public class BajaMapaDeRuta extends javax.swing.JFrame{
	...
	private void initialize() {
		...
		JLabel titulo = new JLabel("Baja Mapa de Ruta");
		...
		btnBuscar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (DTO != null){
					cliente.setText(Controlador.getInstancia().getSucursal(DTO.getIdSucursalOrigen()).getDescripcion() + " a " + Controlador.getInstancia().getSucursal(DTO.getIdSucursalDestino()).getDescripcion());
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
					...
					boolean flag = Controlador.getInstancia().bajaMapaDeRuta(DTO.getId());

					if(flag){
						JOptionPane.showMessageDialog(null,"Se ha dado de baja el Mapa de Ruta", "Baja Mapa de Ruta realizado", JOptionPane.INFORMATION_MESSAGE);
						setVisible(false);
					} else {
						JOptionPane.showMessageDialog(null,"No se ha podido dar de baja el Mapa de Ruta.", "Error", JOptionPane.ERROR_MESSAGE);	
					}
			}
		});
